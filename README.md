...
## 1. Outline
### 1.1. Introduction
[![Build Status](https://github.com/austinibele/cms-backend/workflows/build/badge.svg)](https://github.com/austinibele/cms-backend/actions?query=workflow%3Abuild)
...
Template for a NodeJS Backend Server powered by:
  ...
  - [NestJS](https://nestjs.com): NodeJS Typescript Backend Framework
    - [`@nestia/core`](https://github.com/austinibele/nestia): validation decorators 20,000x faster than `class-validator`
    - [`@nestia/sdk`](https://github.com/austinibele/sdk): SDK and Swagger Documents generator
  - [Prisma](https://www.prisma.io) and [`prisma-markdown`](https://github.com/austinibele/prisma-markdown)

Prior, to making this template opensource, I've prepared a couple of backend projects leveraging this template.
Reading this [README.md](https://github.com/austinibele/cms-backend) document and traveling below example projects, you may understand how to develop the TypeScript backend server with the [nestia](https://github.com/austinibele/nestia) and [safe-typeorm](https://github.com/austinibele/safe-typeorm).

  - [austinibele/fake-iamport-server](https://github.com/austinibele/fake-iamport-server): Fake iamport server, but real SDK
  - [austinibele/fake-toss-payments-server](https://github.com/austinibele/fake-toss-payments-server): Fake toss-payments server, but real SDK

If you have got queries like "building a new type of backend template" or "regarding backend server development using Typescript", feel free to ask it out over [here](https://github.com/austinibele/cms-backend/issues).
...
| Before          | After
|-----------------|----------------------------------------
| austinibele | Your account or corporation name
| cms-backend      | Your own project name
| AUTHOR       | Author name
| db_name      | Database to connnect
| db_schema    | Database schema to use
| db_account   | Database account to use, not root account
| https://github.com/austinibele/cms-backend | Your repository URL
...
When you're planning to deploy the backend server, read the [INFRASTRUCTURE.md](INFRASTRUCTURE.md) file and follow the steps. Following the steps, you may run some executable programs of the [src/executable](src/executable) directory.
  ...
  - [src/](src/): TypeScript Source directory
    - [src/api/](src/api/): Client SDK that would be published to the `@austinibele/cms-backend-api`
      - [**src/api/functional/**](src/api/functional/): API functions generated by the [`nestia`](https://github.com/austinibele/nestia)
...
Also as you can see from the [package.json](package.json) file, this project requires the private npm module `@austinibele`, provided from the Github. Therefore, to develop this backend server, you've configure the `.npmrc` file. Open the below link and complete the configuration.
...
However, [@austinibele](https://github.com/austinibele) does not recommend to writing code down into the [Main Program](#34-main-program) first, without any consideration. Instead, [@austinibele](https://github.com/austinibele) recommends to declare the definition first and implement the [Main Program](#34-main-program) later.
...
To build the SDK in local, just type the `npm run build:sdk` command. The SDK would be generated by [`nestia`](https://github.com/austinibele/nestia), by analyzing source code of the [controller](src/controllers) classes in the compilation level, automatically. Otherwise you want to publish the SDK .ibrary, run the `npm run package:api` command instead.
...
import { IBbsCitizen } from "@austinibele/bbs-api/lib/structures/bbs/actors/IBbsCitizen";
import { IBbsQuestionArticle } from "@austinibele/bbs-api/lib/structures/bbs/articles/IBbsQuestionArticle";
import { IBbsSection } from "@austinibele/bbs-api/lib/api/structures/bbs/systematic/IBbsSection";
...
```bash
# MOVE TO THE cms-backend DIRECTORY
ssh ${dev_address}
...
You know what? All of CI processes, like installing required programs into the clean Ubuntu and compiling and running the Test Automation Program, are defined in the [`.github/workflows/build.yml`](.github/workflows/build.yml) script file. Also <font color="green">passing</font> or <font color="red">failure</font> expressed from the above [badge](https://github.com/austinibele/cms-backend/actions?query=workflow%3Abuild) represents outcome of the CI test.
